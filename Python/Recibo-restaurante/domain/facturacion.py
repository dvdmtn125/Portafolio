...
def calcular_subtotal(cantidades, precios):
...
def calcular_totales(cantidades, precios):
    if cantidades.keys() != precios.keys():
        raise ValueError(
            f"Las categorías de cantidades ({list(cantidades)}) no coinciden "
            f"con las de precios ({list(precios)})."
        )
